...
class ObstacleNode(Node):
    ...
    def scan_callback(self, msg: LaserScan) -> None:
        ...
        if not self.is_valid_scan(msg):
            ...
            return
        # A gap longer than the timeout, or time running backwards (bag replay restarting), drops the
        # tracks and the occupancy state so nothing carries a stale or future timestamp.
    ...
    # 노드 시계의 현재 시각(초). 실센서에서는 벽시계, bag 재생(use_sim_time)에서는 bag 시각을 따른다
