...
sensor_msgs::msg::LaserScan make_scan(const soslab::FrameData& frame,
  const ScanConfig& config, const builtin_interfaces::msg::Time& stamp, double scan_time) {
  if (frame.depth.empty() || frame.depth[0].size() < 2 ||
      frame.intensity.empty() || frame.intensity[0].size() != frame.depth[0].size())
    throw std::invalid_argument("Empty or mismatched GL5 range/intensity arrays");
  if (config.frame_id.empty() || !std::isfinite(config.range_min) || !std::isfinite(config.range_max) ||
      config.range_min < 0 || config.range_max <= config.range_min || config.range_max > 60 ||
      !std::isfinite(config.angle_offset) || !std::isfinite(scan_time) || scan_time < 0)
    throw std::invalid_argument("Invalid scan configuration");
  constexpr double pi = 3.14159265358979323846;
  sensor_msgs::msg::LaserScan scan;
  scan.header.stamp = stamp; scan.header.frame_id = config.frame_id;
  const size_t count = frame.depth[0].size();
  // SDK angles are [-45, 225] degrees; rotate -90 degrees so center is +X.
  scan.angle_min = -0.75 * pi + config.angle_offset;
  scan.angle_increment = 1.5 * pi / (count - 1);
  ...
  scan.range_min = config.range_min; scan.range_max = config.range_max;
  scan.ranges.reserve(count); scan.intensities.reserve(count);
  for (size_t i = 0; i < count; ++i) {
    const float distance = frame.depth[0][i] * 0.001f;
    scan.ranges.push_back(frame.depth[0][i] > 0 && distance >= scan.range_min && distance <= scan.range_max
...
sensor_msgs::msg::PointCloud2 make_cloud(const sensor_msgs::msg::LaserScan& scan) {
  ...
  sensor_msgs::PointCloud2Iterator<float> x(cloud, "x"), y(cloud, "y"), z(cloud, "z"), intensity(cloud, "intensity");
