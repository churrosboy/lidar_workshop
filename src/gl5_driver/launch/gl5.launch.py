...
def generate_launch_description():
    share = get_package_share_directory('gl5_driver')
    ...
    return LaunchDescription([
        DeclareLaunchArgument('params_file', default_value=os.path.join(share, 'config', 'gl5.yaml')),
        ...
        DeclareLaunchArgument('obstacle_params_file', default_value=os.environ.get(
            'GL5_OBSTACLE_PARAMS_FILE', os.path.join(share, 'config', 'obstacles.yaml'))),
        DeclareLaunchArgument('region_file', default_value=os.environ.get(
            'GL5_REGION_FILE', os.path.join(os.getcwd(), 'gl5_region.json'))),
        driver,
        Node(package='gl5_driver', executable='gl5_obstacle_node.py',
        ...
        Node(package='rviz2', executable='rviz2', name='gl5_rviz', output='screen',
             arguments=['-d', os.path.join(share, 'rviz', 'gl5.rviz')],
